...
export default function CommissionsPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">IB Commissions</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Manage and view IB commissions.
        </p>
      </div>

      <Card className="p-6 bg-card border-border">
        <h2 className="text-lg font-semibold mb-4">All Commissions</h2>
        {loading ? (
          <p>Loading...</p>
        ) : (
          <Table>
            ...
            <TableBody>
              {commissions.map((commission) => (
                <TableRow key={commission._id}>
                  ...
                  <TableCell className="text-right">
                    ${commission.brokerage.toFixed(2)}
                  </TableCell>
                  <TableCell>
